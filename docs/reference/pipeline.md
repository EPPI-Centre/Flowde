...
[stopping and resuming](../pipeline/resuming.md) for the complete recovery rules.
...
| `n_jobs`        | Number of worker processes. `1` runs one item at a time. Extraction defaults to `1`; the other stages default to the available CPU count. Set this explicitly when controlling concurrent API requests.                  |
...
Resuming an overlapping slice restores completed answers within that slice. Answers
outside the current slice remain saved, but are not included in the returned
list.
...
into the run's `save_dir`. The run record includes successful
PDFs that produced no images. Missing PNGs cause the whole source PDF to be
extracted again on resume; modified saved PNGs cause an error.
...
See the [classification guide](../pipeline/classification.md) for worked examples.
...
See the [parsing guide](../pipeline/parsing.md) for full, partial and custom-schema
examples.
