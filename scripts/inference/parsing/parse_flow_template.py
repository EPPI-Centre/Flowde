...
DIR_NAME_TO_SAVE_RESULTS = "template_parse_just_flow_results"
PARSED_NODES_DATASET = "template_parse_just_nodes_results"
PARSED_LABELS_DATASET = "template_parse_just_labels_results"
PARSED_ADDITIONAL_TEXTS_DATASET = "template_parse_just_additional_text_results"
...
NODES_DIR = PRED_DIR / PARSED_NODES_DATASET
LABELS_DIR = PRED_DIR / PARSED_LABELS_DATASET
...
nodes, labels and additional text.
I just want you to parse the flow.
...
LABEL:
- In the diagram, you may see text that is not a node, but is intended to be
directly applied to one or many nodes.
For example, you will often see text such as 'Allocation', 'Enrollment',
'Follow-up', 'Analysis' as well as many others.
This text often applies to multiple nodes to categorise them as to what stage
the node belongs to in the particpant flow.
- For each node you have parsed, you should also parse any labels that apply to
it.
- Do not add any nodes as labels.
- Don't label nodes with labels that are not in the image, even if the label
would be appropriate. Your job is to parse the image as it appears, not to add
in any information that is not in the image.
- If there is a title, which appear to apply to all nodes in the diagram, do
not parse it as a label. It will go into the additional_text section.
- As a standard convention, if the node that splits into two different
alllocation branches is labelled either 'Enrollment' or 'Allocation', always
default to labelling it 'Enrollment'

FLOW:
- For each node, state which node(s) (by writing their respective node_numbers)
it points to.
- Just because there isn't an edge between two nodes, doesn't necessarily mean
there isn't a flow betweeen them. You should use your intuition.

ADDITIONAL_TEXT:
- Any text that is neither a node nor a label is additional text.
- Please parse a list of all chunks of additional text, order according to row
major column order, e.g. top to bottom and left to right of the nodes centers.
- Try to use intuition for determining whether additional text should be a
single chunk or multiple.
Some examples: A Title is its own chunk.
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = ArgumentParser(description="Parse flow from extracted images.")
    parser.add_argument(
        "--model",
        default=MODEL,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=NODES_DIR,
        help=f"Directory containing parsed node JSON (default: {NODES_DIR}).",
    )
    parser.add_argument(
        "--labels-dir",
        type=Path,
        default=LABELS_DIR,
        help=f"Directory containing parsed label JSON (default: {LABELS_DIR}).",
    )
    parser.add_argument(
        "--additional-texts-dir",
        type=Path,
        default=ADDITIONAL_TEXTS_DIR,
        help=(
            "Directory containing parsed additional-text JSON "
            f"(default: {ADDITIONAL_TEXTS_DIR})."
        ),
    )
    parser.add_argument(
        "--start-index",
